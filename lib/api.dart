...
Future<Map<String, String>?> getHeader() async {
  return {"Content-type": "application/json", "OS": Platform.operatingSystem, "App-version": "1.0.0", "Authorization": "sdffdsf"};
  // await LocalStorage().readToken() ?? ''
  // };
}
...
//------------------Get Departments---------------------------//
  ...
  Future<GeneralResponse?> checkVersion() async {
    try {
      final response = await http.post(Uri.parse(Urls.checkVersion), body: jsonEncode({}), headers: await getHeader()).timeout(Duration(seconds: 60));
      log('response: ${(response.body)}');
      final responseJson = jsonDecode(response.body) as Map<String, dynamic>;
      return GeneralResponse.fromJson(responseJson);
    } catch (e) {
  ...
  Future<IssueResponse?> getAdminIssues() async {
    try {
      final response = await http.post(Uri.parse(Urls.getAdminIssue), body: jsonEncode({}), headers: await getHeader()).timeout(Duration(seconds: 60));
      log(response.body);
      final responseJson = jsonDecode(response.body) as Map<String, dynamic>;
  ...
  Future<IssueResponse?> getVolIssues() async {
    try {
      final response = await http.post(Uri.parse(Urls.getVolIssue), body: jsonEncode({}), headers: await getHeader()).timeout(Duration(seconds: 60));
...
tokenExpired() async {
  ...
  //     ),
  //     content: Text('This session as expired. Please login again.'),
  //     actions: [
  //       TextButton(
  //         onPressed: () => logout(),
  //         child: Text('Ok'),
