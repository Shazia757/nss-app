...
class Api {
  String baseUrl = kDebugMode ? 'https://nssapi.bvocfarookcollege.com/api' : 'https://nss.noorabiyad.com/api';
...
//------------------Login---------------------------//
  ...
  Future<LoginResponse?> login(Map<String, dynamic> data) async {
    try {
      final response = await http.post(Uri.parse('$baseUrl/login/'), body: jsonEncode(data), headers: headers).timeout(Duration(seconds: 60));
  ...
  Future<Department?> getDepartments() async {
    try {
      final response = await http
          .post(
            Uri.parse('$baseUrl/get_departments/'),
            headers: headers,
          )
